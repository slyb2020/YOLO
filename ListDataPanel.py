...
class ResultDisplayPanel(wx.Panel, listmix.ColumnSorterMixin):
    def __init__(self, parent, log, data=[]):
        wx.Panel.__init__(self, parent, -1, style=wx.BORDER_THEME)
        self.parent = parent
        self.log = log
    ...
    def PopulateList(self):
        ...
        if self.data:
            for i in range(20):
                index = self.list.InsertItem(self.list.GetItemCount(), CLASSES2[i], self.idx1)
                item = self.list.GetItem(index, 1)
                # index=self.data.index(i)
                # self.list.SetItem(index, 0, i)
                self.list.SetItem(index, 1, str(self.data[0][5+i]))
                self.list.SetItem(index, 2, str(self.data[1][5+i]))
                # self.list.SetItemData(index,i[0])
